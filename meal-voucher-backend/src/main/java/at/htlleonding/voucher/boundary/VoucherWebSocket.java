...
public class VoucherWebSocket {
    ...
    // Method to push data to connected WebSocket clients
    public static void pushData(Object data) {
        String jsonData = toJson(data); // Convert data to JSON format if needed
        for (Session session : sessions) {
            try {
                session.getBasicRemote().sendText(jsonData);
                System.out.println("Data sent to client " + session.getId() + ": " + jsonData);
            } catch (IOException e) {
    ...
    // Helper method to convert object to JSON format (if needed)
